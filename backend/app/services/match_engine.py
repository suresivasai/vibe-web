...
async def end_session(
    ...
    """End a session only when the caller is a member of it."""
    try:
        parsed_session_id = UUID(str(session_id))
    except (TypeError, ValueError):
        return None

    query = select(Session).where(Session.id == parsed_session_id)
    if user_id is not None:
        query = query.where((Session.user1_id == user_id) | (Session.user2_id == user_id))
